...
@interface AccountManageBindingController (){
    NSTimer *_timer;//计时器
...
@implementation AccountManageBindingController
...
- (void)createPhoneView{
    ...
    [self.numberText addTarget:self action:@selector(valueChange) forControlEvents:UIControlEventValueChanged];
    self.numberText.keyboardType = UIKeyboardTypeNumberPad;
    self.numberText.placeholder = @"请输入手机号";
    ...
    [self.yzmText addTarget:self action:@selector(valueChange) forControlEvents:UIControlEventValueChanged];
...
- (void)valueChange{
    ...
    if (self.numberText.text.length > 13) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"有这么长的手机号吗- -!!" delegate:self cancelButtonTitle:@"改改~" otherButtonTitles: nil];
        [alert show];
        self.numberText.text = nil;
    }
    if (self.yzmText.text.length > 8) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"你确定验证码有那么长?" delegate:self cancelButtonTitle:@"改改~" otherButtonTitles: nil];
        [alert show];
        self.yzmText.text = nil;
    }
